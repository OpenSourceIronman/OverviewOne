...
class SwiftRadioCommandParser(argparse.ArgumentParser):
	...
	def error(self, message):
		raise SwiftCommandParserError(message)


class SwiftCommandParserError(Exception):
	"""
	Description:
	Parameters:
	Return:
	...
	def get_error_msg(self, added_info = None):
		...
		error_msg = "\n\nSwiftCommandParserError: " + str(self.message)
		if added_info != None:
			error_msg += "\n\n-Additional Info- \n\n" + str(added_info)
...
def get_parser_settings(paramtype):
	...
	if paramtype == "int":
		...
		try:
			raise SwiftCommandParserError("cannot register unrecognized parameter type '%s'"%(paramtype))
		except SwiftCommandParserError, error:
			parser_error = error.get_error_msg()
			raise SwiftCommandParserError(parser_error)
