...
class SwiftCmdhostCommandTable:
	...
	def get_output_list(self, cname):
		...
		Return:
		"""
		if cname in self.command_name_dict:
			command = self.command_name_dict[cname]
		else:
			return None
	...
	def parse_parameter_text(self, command_name, arglist):
		...
		except cmdlinesyntax.SwiftCommandParserError, error:
			parser_error = error.get_error_msg(added_info = parser.format_help())
			raise SwiftCommandTableError(parser_error)
			return None
	...
	def register_command_by_name(self, command_name, description=""):
		...
		if self.verify_command_registered(command_name) == 0:
			...
			raise SwiftCommandTableError("command '%s' is already registered"%(command_name))
			return -1
		...
		if self.verify_command_registered(command_name) == 1:
			...
			if self.verify_parameter_registered(command_name, pname) != 1:
				...
				if registration_complete != 1:
					raise SwiftCommandTableError("parameter registration failure")
					return -3
			else:
				raise SwiftCommandTableError("register_parameter->parameter '%s' is already registered"%(pname))
				return -2
		else:
			raise SwiftCommandTableError("register_parameter->command '%s' does not exist"%(command_name))
			return -1
		return 1
	...
	def register_output(self, command_name, pname=None, ptype=None, pdefault=None, pdesc=None, min_val=None, max_val=None, units=None):
		...
		if self.verify_command_registered(command_name) == 1:
			...
			if self.verify_output_registered(command_name, pname) != 1:
				...
				# raise exception if parameter could not be registered
				if registration_complete != 1:
					raise SwiftCommandTableError("output registration failure")

			else:
				raise SwiftCommandTableError("register_output->output '%s' is already registered"%(pname))

		else:
			raise SwiftCommandTableError("register_output->command '%s' does not exist"%(command_name))
...
class SwiftCommandTableError(RuntimeError):
	"""
	Description:
	Parameters:
	Return:
