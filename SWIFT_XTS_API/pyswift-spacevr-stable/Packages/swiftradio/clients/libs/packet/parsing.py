...
def construct_swiftmsg_buffer(Command_Table, command_name, params_info_dict, escape_code, transid, control_code, buffer_endianess):
	...
	if command_hash == -1:
		raise SwiftRPCParserError("Packet Translator: '" + str(command_name) + "' command name not recognized.")
		swift_msg_buffer = -1
	...
	if params_info_dict != None:
		for param_name in params_info_dict:
			...
			if parameter_hash == -1:
				raise SwiftRPCParserError("Packet Translator: '" + str(param_name) + "' parameter name not recognized.")
				swift_msg_buffer = -1
...
class SwiftRPCParserError(RuntimeError):
	"""
	Description:
	Parameters:
	Return:
