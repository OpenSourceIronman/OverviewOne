...
class SwiftfirmFTP(object):
	...
	"""
	def __init__(self, logger=None, endianess="big"): 
		"""
		Author: S. Alvarado
		Last Updated: 8/7/15
		...
		if (logger == None):	
			logging.basicConfig(format="[SwiftFirmFTP] %(asctime)s %(levelname)s: %(message)s", datefmt='%m/%d/%y %H:%M:%S', level=logging.INFO)
