...
def convert_raw(buf, data_type, order=NETWORK):
	"""
	Converts a string of binary data into a specified data type. See below for a list of data type \
	(dtype) values to choose from.

	:param str buf: string object to perform data conversion.
	:param str order: Endianess of conversion data. See below for valid values.
	:param str data_dtype: Return data type. See below for valid values.

	Valid data conversion types:
	...
	Valid order (Endianess) values:
		- "=" : Native byte order of host system.
		- "<" : Little Endian byte order.
		- ">" : Big Endian byte order.
		- "!" : Network byte order (big-Endian).

	:returns: The value in the specified data format.

	Last Updated: 05/12/16 (SRA)
	"""
	parsed_val = None
	data_length = None
