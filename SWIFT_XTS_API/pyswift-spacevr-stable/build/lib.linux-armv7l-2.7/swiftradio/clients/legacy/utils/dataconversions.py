...
def bytelist_to_uint(bytelist, endianess='little', lsize = None):
	...
	if lsize != None:
		...
		num_bytes = len(bytelist)

	if endianess == 'big':
		bytelist = bytelist[::-1]

	for i in range(num_bytes):
		return_uint += ord(bytelist[i])<<(8*(num_bytes-(i+1)))
...
def bytelist_to_int(bytelist, endianess='little', lsize = None):
	...
	"""
	return_int = bytelist_to_uint(bytelist, endianess, lsize)

	if (lsize == 1) and (return_int > 0xFF/2):
		return_int -= (0xFF + 1)
	elif (lsize == 2) and (return_int > 0xFFFF/2):
		return_int -= (0xFFFF + 1)
	elif (lsize == 3) and (return_int > 0xFFFFFF/2):
		return_int -= (0xFFFFFF + 1)
	elif (lsize == 4) and (return_int > 0xFFFFFFFF/2):
...
def bytelist_to_float(bytelist, endianess='little', lsize = None):
	...
	if endianess == 'big' or endianess == 'network':
		return_float = struct.unpack('<d',"".join(bytelist))
	if endianess == 'little':
		return_float = struct.unpack('!d',"".join(bytelist))

	return_float = return_float[0]
	return return_float
