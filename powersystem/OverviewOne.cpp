...
{ 
  ...
  ThermalKnifePlan.setPowerPlan(0, 30, true);
  ...
  OperationalPlan CommsPlan = OperationalPlan(plan101010, 6);  
  OperationalPlan CameraPlan = OperationalPlan(plan010001, 6);
  OperationalPlan AttitudePlan = OperationalPlan(plan011001, 6);
  ...
  PowerConsumption PowerConsumptionSystems[NUMBER_OF_POWER_DRAW_SYSTEMS] =
    {PowerConsumption("TK1", 0.960, 12.0, 100.0, &AlwaysOn), 
     PowerConsumption("See3CamCU130:0", 0.508, 5.0, 100.0, &CameraPlan),
     PowerConsumption("See3CamCU130:1", 0.508, 5.0, 100.0, &CameraPlan),
     PowerConsumption("SWIFT-XTS", 0.188, 7.4, 100.0, &CommsPlan),
  ...
  bool previousIlluminationState = false;
  int illuminationStateChange = NO_CHANGE;
  
  for(int orbitNumber = 0; orbitNumber < FINAL_ORBIT; orbitNumber++){
    
    for(int orbitalTime = 0; orbitalTime < FULL_ORBIT; orbitalTime += ORBIT_PLANNING_RESOLUTION ){
      if (orbitalTime < HALF_ORBIT) illuminationState = true;
      else if (orbitalTime >= HALF_ORBIT) illuminationState = false;
      
      if (previousIlluminationState == true && illuminationState == false){
        illuminationStateChange = SUNSET; 
 	  }
      else if (previousIlluminationState == false && illuminationState == true){
      ...
      for(int i = 0; i < NUMBER_OF_POWER_DRAW_SYSTEMS; i++){
	    PowerConsumptionSystems[i].update(ORBIT_PLANNING_RESOLUTION, orbitNumber, orbitalTime, illuminationStateChange);
	    illuminationStateChange = NO_CHANGE; 	    
	  }//END INNER FOR LOOP
	  
	  previousIlluminationState = illuminationState;
	 
	  cout << "Total power consumption at orbit mintue " << orbitalTime << " was " << totalConsumption[POWER] << " Wh" << endl;
	  cout << "Total energy consumption at orbit mintue "<< orbitalTime << " was " << totalConsumption[ENERGY] << " Ah" << endl << endl;
    ...
    //~ for(int i = 0; i < NUMBER_OF_POWER_DRAW_SYSTEMS; i++){
	  //~ PowerConsumptionSystems[i].update(0, HALF_ORBIT, (orbitNumber*2));
	  //~ totalConsumption[POWER] += PowerConsumptionSystems[i].getPowerConsumption();
	  //~ totalConsumption[ENERGY]+= PowerConsumptionSystems[i].getEnergyConsumption(); 
	//~ }
	...
	//usleep(500000); //Delay 500 milliseconds
