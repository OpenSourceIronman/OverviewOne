 ...
 * @author Blaze Sanders 
 */
void PowerConsumption::update(int stepSize, int orbitNumber, int orbitMinute, int illuminationChange){
  
  switch(illuminationChange){
    case SUNRISE:
      cout << endl << "SUNRISE of orbit #" << orbitNumber << endl;
      break;
    case NO_CHANGE:
      break;
    case SUNSET:
  ...
  if (opPlan.getPowerPlan(orbitNumber, orbitMinute)){ 
    if(DEBUG){
      cout << partNumber << " is on (orbit number " << orbitNumber;
      cout << " and orbit minute " << orbitMinute << ")" << endl;
    }
    
    double ratio = stepSize/60.0;
    
    energyConsumed += (currentDraw * ratio);
    powerConsumed  += (powerRating * ratio);
